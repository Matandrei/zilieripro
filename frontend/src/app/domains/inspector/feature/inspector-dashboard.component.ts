...
@Component({
  ...
  template: `
    <div class="max-w-7xl mx-auto">
      <div class="mb-6">
        <h1 class="text-3xl font-bold tracking-tight text-foreground">Dashboard Inspector</h1>
        <p class="text-sm text-muted-foreground mt-1">Statistici agregate cross-beneficiar pentru Inspectoratul de Stat al Muncii.</p>
      </div>
      ...
      @if (loading()) {
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          ...
          <div class="rounded-xl bg-card ring-1 ring-foreground/10 p-4">
            <div class="text-xs uppercase tracking-wider text-muted-foreground">Beneficiari distincti</div>
            <div class="mt-1 text-2xl font-bold">{{ uniqueBeneficiaries() }}</div>
          </div>
          <div class="rounded-xl bg-card ring-1 ring-foreground/10 p-4">
            <div class="text-xs uppercase tracking-wider text-muted-foreground">Zilieri distincti</div>
            <div class="mt-1 text-2xl font-bold">{{ uniqueWorkers() }}</div>
          </div>
          <div class="rounded-xl bg-card ring-1 ring-foreground/10 p-4">
            <div class="text-xs uppercase tracking-wider text-muted-foreground">Total sume (net)</div>
        ...
        <div class="bg-card rounded-xl ring-1 ring-foreground/10 p-5 mb-6">
          <h2 class="text-sm font-semibold mb-3">Vouchere dupa statut</h2>
        ...
        <!-- Per district -->
        <div class="bg-card rounded-xl ring-1 ring-foreground/10 p-5 mb-6">
          <h2 class="text-sm font-semibold mb-3">Vouchere pe regiuni</h2>
          <div class="space-y-2">
            @for (r of districtStats(); track r.district) {
              <div class="flex items-center gap-3">
                ...
                <div class="w-24 text-right text-xs text-muted-foreground">{{ r.totalNet }} MDL</div>
              </div>
            }
          </div>
        </div>
        ...
        <div class="bg-card rounded-xl ring-1 ring-foreground/10 p-5">
          <h2 class="text-sm font-semibold mb-3">Acțiuni rapide</h2>
          <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
            <a routerLink="/vouchers"
              ...
              <div>
                <div class="font-semibold">Verifica vouchere</div>
                <div class="text-xs text-muted-foreground">Cauta cross-beneficiar dupa Cod/IDNP/data</div>
              </div>
            </a>
            <a routerLink="/workers"
              ...
              <div>
                <div class="font-semibold">Verifica lucratori</div>
                <div class="text-xs text-muted-foreground">Toti zilierii din sistem</div>
              </div>
            </a>
            <a routerLink="/reports"
              ...
              <div>
                <div class="font-semibold">Rapoarte statistice</div>
                <div class="text-xs text-muted-foreground">Agregate pe perioada si raion</div>
              </div>
            </a>
          </div>
        </div>
